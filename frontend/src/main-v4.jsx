...
function App() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => setSellerNotice(''), 3000);
    return () => clearTimeout(timer);
  }, [sellerNotice]);
  ...
  function cancelEdit(resetForm) {
    setEditingProduct(null);
    resetForm();
    setSellerNotice('Edición cancelada. El formulario volvió a modo agregar producto.');
...
function SellerPanel({ categories, products, orders, summary, notice, saveProduct, editingProduct, setEditingProduct, cancelEdit, toggleProduct, updateOrderStatus, showOrder }) {
...
function ProductForm({ categories, saveProduct, editingProduct, cancelEdit }) {
  ...
  return <div className="card shadow-sm"><div className="card-body"><div className="d-flex justify-content-between align-items-center mb-3"><h3 className="h5 mb-0">{editingProduct ? 'Editar producto' : 'Publicar producto'}</h3>{editingProduct && <button type="button" className="btn btn-sm btn-outline-secondary" onClick={handleCancel}>Cancelar</button>}</div><form className="row g-2" onSubmit={submit}><div className="col-md-6"><input className="form-control" name="name" value={form.name} onChange={change} placeholder="Nombre" required /></div><div className="col-md-3"><input className="form-control" name="price" type="number" value={form.price} onChange={change} placeholder="Precio" required /></div><div className="col-md-3"><input className="form-control" name="stock" type="number" value={form.stock} onChange={change} placeholder="Stock" required /></div><div className="col-md-4"><select className="form-select" name="categoryId" value={form.categoryId} onChange={change}>{categories.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div><div className="col-md-8"><input className="form-control" name="imageUrl" value={form.imageUrl} onChange={change} placeholder="URL imagen" /></div><div className="col-12"><textarea className="form-control" name="description" value={form.description} onChange={change} placeholder="Descripción" required /></div><div className="col-12"><button className="btn btn-primary">{editingProduct ? 'Actualizar producto' : 'Agregar producto'}</button></div></form></div></div>;
}
